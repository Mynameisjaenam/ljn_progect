	...
	<header class="masthead">
		<div id="con">
			<div id="rental">
				<div id="rental_form">
					<form>
						<h3 class="login" style="letter-spacing: -1px; color:white;">나의 게시글 조회</h3>
						<br>
						<hr>
						<br>
						<div id="mainHide">
							<table class="table" id="table" style='width: 1000px'>
								...
								<tbody>
									<c:forEach var="myList" items="${myBoardList}">
										<tr style="color:white">
											<td>${myList.boardNo}</td>
											<td>${myList.boardType}</td>
											<td><a href="selectMyBoard.do?boardNo=${myList.boardNo}" >${myList.title}</a></td>
				            ...
				            <nav aria-label="Page navigation" style="text-align: center;">         
				               <ul class="pagination justify-content-center">
				                  <c:if test="${firstPage > pageList }">
				                     <li><a class="page-link" href="myBoardList.do?viewPage=${firstPage - pageList}">이전</a></li>
				                  </c:if>   
				                  <c:forEach var="i" begin="${firstPage}" end="${lastPage}">                                                
				                     <li><a class="page-link" href="myBoardList.do?viewPage=${i}">${i}</a></li>
				                  </c:forEach>
				                  <c:if test="${lastPage < totalPage}">
				                     <li><a class="page-link" href="myBoardList.do?viewPage=${firstPage + pageList}">다음</a></li>
				                  </c:if>
				               </ul>      
				            </nav> 
				         </div>
						...
						<%
							if(userId != null) {
						%>
						<input type="button" style="width:100px; height:50px; background-color: #CC9966;" class="btn btn-primary btn-sm" onclick="location.href='boardForm.do'" value="등록" />
						<%
							}
						%>
						</div>
