	...
	<header class="masthead">
		<div id="con">
			<div id="rental">
				<div id="rental_form">
					<form>
						<h3 class="login" style="letter-spacing: -1px; color:white;">전체 문의내역</h3>
						<br>
						<hr>
						<br>
						<div id="mainHide">
							<table class="table" id="table" style='width: 1000px'>
								<thead style="background-color: #CC9966">
									<tr>										
										...
										<th scope="col">문의취소</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="adminlist" items="${adminQuestionList}">
										<tr style="color:white">
											<td>${adminlist.questionNo}</td>
											<td><a href="adminselectQuestion.do?questionNo=${adminlist.questionNo}" >${adminlist.title}</a></td>
											<td>${adminlist.content}</td>
											<td>${adminlist.userId}</td>											
											<td>${adminlist.createDate}</td>
											<td><input type="button" style="width:80px; height:40px; background-color: #CC9966;" class="btn btn-primary btn-sm" onclick="adminquestiondelete(${adminlist.questionNo})" value="문의취소" /></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
						...
						<input type="button" style="width:100px; height:50px; background-color: #CC9966;" class="btn btn-primary btn-sm" onclick="location.href='adminAnswerList.do'" value="답글내역조회" />
