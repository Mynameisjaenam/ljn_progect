...
function droneDelete(a) {
    var dNo = a
   
   $.ajax({
      url:'droneDeleteConfirm.do',
      type:'POST',
      data:{         
         droneNo:dNo
      },
      ...
      success: function(data) {
         if(data == "ok"){
            ...
            const url = "http://localhost:8800/ljn/droneDelete.do";
	...
	<header class="masthead">
		<div id="con">
			<div id="rental">
				<div id="rental_form">
					<form>
						<h3 class="login" style="letter-spacing: -1px; color:white;">드론전체조회</h3>
						<br>
						<hr>
						<br>
						<div id="mainHide">
				            ...
				            <nav aria-label="Page navigation" style="text-align: center;">         
				               <ul class="pagination justify-content-center">
				                  <c:if test="${firstPage > pageList }">
				                     <li><a class="page-link" href="droneList.do?viewPage=${firstPage - pageList}">이전</a></li>
				                  </c:if>   
				                  <c:forEach var="i" begin="${firstPage}" end="${lastPage}">                                                
				                     <li><a class="page-link" href="droneList.do?viewPage=${i}">${i}</a></li>
				                  </c:forEach>
				                  <c:if test="${lastPage < totalPage}">
				                     <li><a class="page-link" href="droneList.do?viewPage=${firstPage + pageList}">다음</a></li>
				                  </c:if>
				               </ul>      
				            </nav> 
				         </div>
